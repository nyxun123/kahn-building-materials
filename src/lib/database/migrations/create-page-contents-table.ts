 ...
 * 创建页面内容表迁移
 */
export async function createPageContentsTable() {
  await this.run(`
  ...
  await this.run('CREATE INDEX IF NOT EXISTS idx_page_contents_page_key ON page_contents(page_key)');
  await this.run('CREATE INDEX IF NOT EXISTS idx_page_contents_section_key ON page_contents(section_key)');
  await this.run('CREATE INDEX IF NOT EXISTS idx_page_contents_is_active ON page_contents(is_active)');
  await this.run('CREATE INDEX IF NOT EXISTS idx_page_contents_sort_order ON page_contents(sort_order)');
  await this.run('CREATE INDEX IF NOT EXISTS idx_page_contents_updated_at ON page_contents(updated_at)');
